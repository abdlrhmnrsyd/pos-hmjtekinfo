    ...
    <div className="space-y-6 pb-12 max-w-7xl">
      ...
      <div className="flex items-end justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-xs text-white/25 mt-0.5">Log semua penjualan beserta kasir yang menangani.</p>
        </div>
        {/* Stats pills */}
      ...
      <div className={surface}>
        <div className="p-5 border-b border-white/[0.06] flex items-center justify-between">
          <div>
            <p className="text-sm font-semibold text-white/80">Semua Transaksi</p>
            <p className="text-[10px] text-white/25 mt-0.5">Termasuk informasi kasir yang menangani.</p>
          </div>
          <p className="text-[10px] text-white/20 font-mono">{filtered.length} hasil</p>
        </div>

        <Table>
          <TableHeader>
            <TableRow className="border-white/[0.05] hover:bg-transparent">
              <TableHead className="h-10 px-5 text-[9px] font-semibold text-white/25 uppercase tracking-widest w-[110px]">ID</TableHead>
              <TableHead className="h-10 text-[9px] font-semibold text-white/25 uppercase tracking-widest">Waktu</TableHead>
              <TableHead className="h-10 text-[9px] font-semibold text-white/25 uppercase tracking-widest">Kasir</TableHead>
              <TableHead className="h-10 text-[9px] font-semibold text-white/25 uppercase tracking-widest">Bayar</TableHead>
              <TableHead className="h-10 text-[9px] font-semibold text-white/25 uppercase tracking-widest">Pesanan</TableHead>
              <TableHead className="h-10 text-right px-5 text-[9px] font-semibold text-white/25 uppercase tracking-widest">Total</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {loading ? (
              ...
              <TableRow className="border-white/[0.04] hover:bg-transparent">
                <TableCell colSpan={6} className="text-center h-32">
                  <p className="text-xs text-white/20">Tidak ada transaksi ditemukan.</p>
                </TableCell>
              </TableRow>
            ) : filtered.map(trx => (
              <TableRow key={trx.id} className="border-white/[0.04] hover:bg-white/[0.02] transition-colors group">
                ...
                <TableCell>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[11px] font-semibold text-white/60 group-hover:text-white/80 transition-colors whitespace-nowrap">
                      {trx.profiles?.name || <span className="text-white/20 italic">Tidak diketahui</span>}
                    </span>
                  </div>
                </TableCell>
                ...
                <TableCell className="max-w-[220px]">
                  <p className="text-[10px] text-white/30 group-hover:text-white/55 transition-colors truncate">
        ...
        {filtered.length > 0 && !loading && (
          <div className="px-5 py-3 border-t border-white/[0.06] flex items-center justify-between">
            <span className="text-[10px] text-white/25">{filtered.length} transaksi ditampilkan</span>
